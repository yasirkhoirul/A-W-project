...
void main() {
  ...
  group("signin with email usecase", () {
    ...
    test("test singin berhasil dan mengembalikan uid serta email", () async {
      ...
      result.fold(
        (failure) =>
            fail('Should return User, got failure: ${failure.message}'),
        (user) {
    ...
    test(
      "test password kurang dari 6 karakter mengembalikan failure",
      () async {
        // Arrange
    ...
    test("test repository mengembalikan failure", () async {
      // Arrange
      when(mockAuthRepository.signInWithEmail(email, password)).thenAnswer(
        (_) async =>
            const Left<Failure, User>(AuthFailure('Email atau password salah')),
      );
